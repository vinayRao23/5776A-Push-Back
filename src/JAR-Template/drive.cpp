...
Drive::Drive(enum ::drive_setup drive_setup, motor_group DriveL,
  ...
  if (drive_setup == TANK_ONE_FORWARD_ENCODER ||
      drive_setup == TANK_ONE_FORWARD_ROTATION ||
      drive_setup == ZERO_TRACKER_ODOM) {
    odom.set_physical_distances(ForwardTracker_center_distance, 0);
  }
  if (drive_setup == TANK_ONE_SIDEWAYS_ENCODER ||
      drive_setup == TANK_ONE_SIDEWAYS_ROTATION ||
 ...
 * The robot exits when error is less than settle_error for a duration of
 * settle_time, or if the function has gone on for longer than timeout.
 *
...
float Drive::get_ForwardTracker_position() {
  if (drive_setup == ZERO_TRACKER_ODOM ||
      drive_setup == TANK_ONE_SIDEWAYS_ENCODER ||
      drive_setup == TANK_ONE_SIDEWAYS_ROTATION) {
    return (get_right_position_in());
  }
  if (drive_setup == TANK_ONE_FORWARD_ENCODER ||
      drive_setup == TANK_TWO_ENCODER || drive_setup == HOLONOMIC_TWO_ENCODER) {
 ...
 * Uses the double-PID method, with one for driving and one for heading
 * correction. The drive error is the euclidean distance to the desired point,
 * and the heading error is the turn correction from the current heading to the
 * desired point. Uses optimizations like driving backwards whenever possible
 * and scaling the drive output with the cosine of the angle to the point.
 *
...
void Drive::drive_to_point(float X_position, float Y_position) {
  drive_to_point(X_position, Y_position, drive_min_voltage, drive_max_voltage,
                 heading_max_voltage, drive_settle_error, drive_settle_time,
                 drive_timeout, drive_kp, drive_ki, drive_kd, drive_starti,
                 heading_kp, heading_ki, heading_kd, heading_starti);
}
...
void Drive::drive_to_point(float X_position, float Y_position,
                           float drive_min_voltage, float drive_max_voltage,
...
void Drive::drive_to_point(float X_position, float Y_position,
                           ...
                           float heading_max_voltage, float drive_settle_error,
                           float drive_settle_time, float drive_timeout,
                           float drive_kp, float drive_ki, float drive_kd,
                           float drive_starti, float heading_kp,
                           float heading_ki, float heading_kd,
                           float heading_starti) {
  PID drivePID(
  ...
  float start_angle_deg = to_deg(
      atan2(X_position - get_X_position(), Y_position - get_Y_position()));
  PID headingPID(start_angle_deg - get_absolute_heading(), heading_kp,
                 heading_ki, heading_kd, heading_starti);
  bool line_settled = false;
  bool prev_line_settled =
      is_line_settled(X_position, Y_position, start_angle_deg, get_X_position(),
                      get_Y_position());
  while (!drivePID.is_settled()) {
    line_settled = is_line_settled(X_position, Y_position, start_angle_deg,
    ...
    drive_with_voltage(left_voltage_scaling(drive_output, heading_output),
                       right_voltage_scaling(drive_output, heading_output));
    task::sleep(10);
  }
}
 ...
 * by the same distance as the robot's distance to the target, times the lead.
 * The robot always tries to go to the carrot, which is constantly moving, and
 * the robot eventually gets into position. The heading correction is optimized
 * to only try to reach the correct angle when drive error is low, and the robot
 * will drive backwards to reach a pose if it's faster. .5 is a reasonable value
 * for the lead. The setback parameter is used to glide into position more
 * effectively. It is the distance back from the target that the robot tries to
 * drive to first.
 *
 * @param X_position Desired x position in inches.
 * @param Y_position Desired y position in inches.
 * @param angle Desired orientation in degrees.
 * @param lead Constant scale factor that determines how far away the carrot
 * point is.
 * @param setback Distance in inches from target by which the carrot is always
 * pushed back.
 * @param drive_min_voltage Minimum voltage on the drive, used for chaining
 * movements.
 */

void Drive::drive_to_pose(float X_position, float Y_position, float angle) {
  drive_to_pose(X_position, Y_position, angle, boomerang_lead,
                boomerang_setback, drive_min_voltage, drive_max_voltage,
                heading_max_voltage, drive_settle_error, drive_settle_time,
                drive_timeout, drive_kp, drive_ki, drive_kd, drive_starti,
                heading_kp, heading_ki, heading_kd, heading_starti);
}
...
void Drive::drive_to_pose(float X_position, float Y_position, float angle,
                          float lead, float setback, float drive_min_voltage) {
  drive_to_pose(X_position, Y_position, angle, lead, setback, drive_min_voltage,
                drive_max_voltage, heading_max_voltage, drive_settle_error,
                drive_settle_time, drive_timeout, drive_kp, drive_ki, drive_kd,
                drive_starti, heading_kp, heading_ki, heading_kd,
                heading_starti);
}
...
void Drive::drive_to_pose(float X_position, float Y_position, float angle,
  ...
  float target_distance =
      hypot(X_position - get_X_position(), Y_position - get_Y_position());
  PID drivePID(target_distance, drive_kp, drive_ki, drive_kd, drive_starti,
               drive_settle_error, drive_settle_time, drive_timeout);
  PID headingPID(to_deg(atan2(X_position - get_X_position(),
                              Y_position - get_Y_position())) -
                     get_absolute_heading(),
                 heading_kp, heading_ki, heading_kd, heading_starti);
  bool line_settled = is_line_settled(X_position, Y_position, angle,
                                      get_X_position(), get_Y_position());
  bool prev_line_settled = is_line_settled(X_position, Y_position, angle,
                                           get_X_position(), get_Y_position());
  bool crossed_center_line = false;
  bool center_line_side = is_line_settled(X_position, Y_position, angle + 90,
                                          get_X_position(), get_Y_position());
  bool prev_center_line_side = center_line_side;
  while (!drivePID.is_settled()) {
    line_settled = is_line_settled(X_position, Y_position, angle,
                                   get_X_position(), get_Y_position());
    if (line_settled && !prev_line_settled) {
      break;
    }
    ...
    center_line_side = is_line_settled(X_position, Y_position, angle + 90,
                                       get_X_position(), get_Y_position());
    ...
    float carrot_X =
        X_position - sin(to_rad(angle)) * (lead * target_distance + setback);
    float carrot_Y =
        Y_position - cos(to_rad(angle)) * (lead * target_distance + setback);

    float drive_error =
        hypot(carrot_X - get_X_position(), carrot_Y - get_Y_position());
    float heading_error =
        reduce_negative_180_to_180(to_deg(atan2(carrot_X - get_X_position(),
                                                carrot_Y - get_Y_position())) -
                                   get_absolute_heading());

    if (drive_error < drive_settle_error || crossed_center_line ||
        drive_error < setback) {
      heading_error =
          reduce_negative_180_to_180(angle - get_absolute_heading());
...
void Drive::holonomic_drive_to_pose(float X_position, float Y_position) {
  holonomic_drive_to_pose(X_position, Y_position, get_absolute_heading(),
...
void Drive::holonomic_drive_to_pose(float X_position, float Y_position,
                                    float angle) {
  holonomic_drive_to_pose(X_position, Y_position, angle, drive_max_voltage,
...
void Drive::holonomic_drive_to_pose(
  ...
  PID drivePID(
      ...
      drive_kp, drive_ki, drive_kd, drive_starti, drive_settle_error,
      drive_settle_time, drive_timeout);
  PID turnPID(angle - get_absolute_heading(), heading_kp, heading_ki,
              heading_kd, heading_starti, turn_settle_error, turn_settle_time,
              turn_timeout);
...
/**
 * Controls a chassis with left stick throttle and strafe, and right stick
 * turning. Default deadband is 5.
 */
...
void Drive::control_tank() {
  ...
  DriveL.spin(reverse, to_volt(leftthrottle), volt);
  DriveR.spin(reverse, to_volt(rightthrottle), volt);
}
